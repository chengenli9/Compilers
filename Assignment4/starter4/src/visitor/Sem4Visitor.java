...
public class Sem4Visitor extends Visitor
{
    ...
    private MethodDecl lookupMethod(ClassDecl cd, String name)
    {
        while (cd != null) {
            MethodDecl m = cd.methodEnv.get(name);
            if (m != null) return m;
            cd = cd.superLink;
        }
        return null;
    ...
    //     return null;
    // }
// verify overriding method matches superclass signature
    private void checkOverride(MethodDecl n)
    {
        if (currentClass.superLink == null) return;
        // find same-named method in superclass chain
        MethodDecl superMeth = lookupMethod(currentClass.superLink, n.name);
        if (superMeth == null) return;
        // link to superclass method
        n.superMethod = superMeth;

        // void vs non-void mismatch
        boolean nIsVoid = (n instanceof MethodDeclVoid);
        boolean superIsVoid = (superMeth instanceof MethodDeclVoid);
        if (nIsVoid != superIsVoid) {
            errorMsg.error(n.pos, CompError.ReturnOverride());
            return;
        }
        // return types must match exactly
        if (!nIsVoid) {
            Type nRtn = ((MethodDeclNonVoid)n).rtnType;
            Type superRtn = ((MethodDeclNonVoid)superMeth).rtnType;
            if (!nRtn.equals(superRtn)) {
                errorMsg.error(n.pos, CompError.ReturnOverride());
                return;
            }
        }
        ...
        if (n.params.size() != superMeth.params.size()) {
            errorMsg.error(n.pos, CompError.NumArgsOverride());
            return;
        }

        // each param type must match exactly
        for (int i = 0; i < n.params.size(); i++) {
            VarDecl p = (VarDecl)n.params.get(i);
            Type pType = p.type;
            Type spType = ((VarDecl)superMeth.params.get(i)).type;
            if (!pType.equals(spType)) {
                errorMsg.error(p.type.pos, CompError.ArgTypeOverride());
                return;
            }
        }
    }
    ...
    public Object visit(MethodDeclNonVoid n)
    {
        ...
        n.params.accept(this);
        n.stmts.accept(this);
        // typecheck return expression against declared return type
        Type rtnExpType = (Type)n.rtnExp.accept(this);
        if (rtnExpType != null && !subtype(rtnExpType, n.rtnType)) {
